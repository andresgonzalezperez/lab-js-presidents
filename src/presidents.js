...
function getDemocraticPresidents(presidentsArr) {
  return presidentsArr.filter(democPres => democPres.party === "Democratic")
}
...
function  countYearsInOffice(presidentsArr) {
  const totalYears = presidentsArr.reduce ((acc, currentPres)=>{
    if (currentPres.leftOffice === null) return acc;

    const yearsServed = currentPres.leftOffice - currentPres.tookOffice;
...
function sortPresidentsByBirthYear(presidentsArr) {
  const newPresidentsArr = presidentsArr.sort((a, b) => a.birthYear - b.birthYear);
  return newPresidentsArr;
} 
...
function getPresidentsBornAfter(presidentsArr, year) {
  ...
  return presidentsArr.filter(onePres => onePres.birthYear > year)
}
...
function countRepublicanPresidents(presidentsArr) {
  return presidentsArr.reduce((acc, currentPres)=>{
    if (currentPres.party === "Republican") {
      return acc + 1;
    }
    return acc;
  },0)
...
function sortPresidentsByName(presidentsArr) {
  return presidentsArr.sort((curr, next)=>{
